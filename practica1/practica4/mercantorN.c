...
void proc(int proc_num) {
	...
	while(!(shared->start_all)); //AQUI SE TIENE ESPERA OCUPADA
...
int i;
	...
	shared->start_all=1; //Aqui se envian NPROCS mensajes no bloqueantes
	
	// Espera a que todos los procesos terminen su cálculo
	while (shared->proc_count != NPROCS) {} //Aqui se hacen NPROCS recieves bloqueantes
